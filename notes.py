...
class Note:
    ...
    def __init__(self, name=None, position=None, prefer_sharps=None):
        ...
        if name is not None:
            ...
            if prefer_sharps is None:
                self.prefer_sharps = False # default
            else:
                self.prefer_sharps = prefer_sharps
            # log(f'Initialising Note with position: {position}')
            self.position = position
            self.name = accidental_note_name(position, prefer_sharps=self.prefer_sharps)
        ...
        self.minor = self.minor_triad = self.minor3 = self.min3 = self.m = self.min
...
##### OctaveNote specific helper/conversion functions:
...
def note_name(inp, prefer_sharps=False):
...
class OctaveNote(Note):
    ...
    def __init__(self, name=None, value=None, pitch=None, prefer_sharps=None):
        ...
        if name is not None:
            ...
            chroma, self.octave = parse_octavenote_name(name)
        ...
        # init as parent Note class:
        super().__init__(position=self.position, prefer_sharps = prefer_sharps)

        # functions as a Note in every respect,
        # except also having .octave, .value, .pitch attrs defined
        # and its addition/subtraction operators etc respect value as well as position
    ...
    def __eq__(self, other):
        """Compares with another Note, returns True if both have the same position
        (but DISREGARDS note value)"""
        assert isinstance(other, Note), "OctaveNotes can only be equal to other notes"
        return self.position == other.position

    def __str__(self):
        # note_name = accidental_note_name(self.position, prefer_sharps=PREFER_SHARPS)
