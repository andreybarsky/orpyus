...
class Scale:
    def __init__(self, name=None, intervals=None, factors=None, alterations=None, chromatic_intervals=None, mode=1):
        ...
        self.scale = self # pointer to a guaranteed Scale object (that is overwritten by Key)
    ...
    def extension(self):
        ...
        if self in scale_extensions:
            return scale_extensions[self]
        else:
    ...
    def extensions(self):
        ...
        if self.extension in scale_extensions:
            return [self.extension, self.extension.extension]
        elif self in scale_extensions:
    ...
    def get_pairwise_intervals(self):
        pairwise = {}
        # scale consonances are counted with both left and right padding:
        padded_intervals = self.intervals.pad(left=True, right=True)
        for i in range(len(padded_intervals)):
            for j in range(i+1, len(padded_intervals)):
                left, right = padded_intervals[i], padded_intervals[j]
                diff = right - left
                pairwise[(left, right)] = diff
        return pairwise
    @property
    def pairwise_intervals(self):
        return self.get_pairwise_intervals()

    def get_pairwise_consonances(self, extra_degrees=None, temperament=None):
        """returns a dict that keys (interval, interval) pairs to the consonances of the
            relative intervals between those two.
        optionally, extra_degrees can be a list of integers, indicating the scale degrees
        ...
        if extra_degrees is not None:
            extra_intervals = [self.degree_intervals[d]  for d in extra_degrees if d in self.degrees]
        ...
        for pair, diff in pw_intervals.items():
            ...
            # if extra degrees are required: add them here:
            if extra_degrees is not None:
    ...
    def get_consonance(self, temperament=None, raw=False, cached=True):
        ...
        if (cached) and ((temperament,self.scale) in cached_consonances):
            raw_cons = cached_consonances[(temperament,self.scale)]
        else:
            cons_list = list(self.get_pairwise_consonances(extra_degrees=[1,1,3,4,5], temperament=temperament).values())
            raw_cons = sum(cons_list) / len(cons_list)**1.1 # slightly downweight shorter scales
            if settings.DYNAMIC_CACHING:
                cached_consonances[(temperament,self.scale)] = raw_cons

        if raw:
            return round(raw_cons,3)
        else:
            # rescale according to observed min/max raw cons values

            # the raw consonance comes out as maximum=0.49 for the most consonant scale (minor pentatonic)
            # and 0.381 for the most dissonant scale (diminished minor pentatonic)
            ...
            max_cons = 0.5
            min_cons = 0.38
            # min_cons = -2.12 # under equal temperament tuning
    ...
    def __neg__(self):
        """ - operator returns the parallel major/minor of a scale"""
    ...
    def play(self, on='G3', up=True, down=True, **kwargs):
        ...
        notes_down = notes_up[-2::-1] # notes descending but not including upper tonic
        full_notes = notes_up if up else notes.NoteList([notes_up[-1]], strip_octave=False)
        if down:
    ...
    _marker = settings.MARKERS['Scale']
    _chromatic_brackets = settings.BRACKETS['chromatic_intervals']
...
class ScaleFactors(Factors):
    ...
    _brackets = settings.BRACKETS['ScaleFactors']
    ...
    def _parse_out_chromatic_factors(self, inp_string):
        ...
        lb, rb, = settings.BRACKETS['chromatic_intervals']
...
class ScaleDegree(int):
    ...
    _diacritic = settings.DIACRITICS['ScaleDegree']
...
class ScaleChord(AbstractChord):
    ...
    def __init__(self, *args, scale=None, degree=None, factor=None, _init_abs=True, **kwargs):
        ...
        if len(args) == 1 and isinstance(args[0], str):
            ...
            if parsing.begins_with_roman_numeral(name):
                ...
                if scale is None:
                    # detect explicit flattened/minor degrees:
                    if flattened_root and degree in (2.5, 5.5, 6.5):
                        # this is a bIII, bVI or bVII with unspecified scale,
                        # which means we should assume minor scale and quietly correct
                        scale = MinorScale
                        degree = ceil(degree)
                    else:
    ...
    def get_numeral(self, modifiers=True, as_string=False, marks=settings.DEFAULT_PROGRESSION_MARKERS, diacritics=settings.DEFAULT_PROGRESSION_DIACRITICS):
        ...
        if not self.root_in_scale:
            ...
            if sharp_factor in self.scale.factors and sharp_factor != 1:
                ...
                lb, rb = settings.BRACKETS['non_key_chord_root']
        ...
        if (not self.notes_in_scale):
            ...
            if self.scale == NaturalMajor:
                related_scales = [# 'natural minor',
                                  ...
                                  'lydian', 'mixolydian']
                rel_scalename_marks = {scale_name:settings.SCALE_MARKS[scale_name] for scale_name in related_scales}
                ...
                if self.scale.has_parallel():
                    rel_scale_marks = {self.scale.parallel: settings.SCALE_MARKS['parallel']}
                else:
                    rel_scale_marks = {}
            ...
            if (chord_in_related_scale) and (not marks):
                ...
                diacritic = settings.DIACRITICS['chord_in_related_scale']
            else:
                diacritic = settings.DIACRITICS['chord_not_in_scale']
    ...
    def short_name(self):
        ...
        return f'{AbstractChord.get_short_name(self)} {self.mod_numeral}'
...
#### now we import the scale definitions described in string form
#### from .defines, and instantiate them as ScaleFactors

#from .defines.define_scales import base_scale_defines

base_scale_factor_names = {ScaleFactors(defn): names for defn,names in base_scale_defines.items()}
...
#### imported dict 'base_scale_mode_names' maps
#### base scale names to dicts that map
#### scale degrees to the modes of that base scale
...
#from .defines.scales import scale_name_replacements
...
# 'exotic' scales:
NeapolitanMajor = Scale('neapolitan major')
NeapolitanMinor = Scale('neapolitan minor')
DoubleHarmonic = ByzantineScale = ArabicScale = GypsyMajor = Scale('double harmonic')
HungarianMinor = GypsyMinor = EgyptianMinor = Scale('hungarian minor')
MiyakoBushi = Scale('miyako-bushi')
...
melodic_scales = [MelodicMajor, MelodicMinor]
...
if settings.PRE_CACHE_SCALES:
    temperament = tuning.get_temperament('CONSONANCE')
    cached_consonances.update({(temperament,c): c.get_consonance(raw=True) for c in common_base_scales})
